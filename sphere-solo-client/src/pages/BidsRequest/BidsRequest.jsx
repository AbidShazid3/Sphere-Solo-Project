...
const BidsRequest = () => {
    const [BidsRequest, refetch] = useBidRequest();
    ...
    const { mutateAsync } = useMutation({
        mutationFn: async ({id, status}) => {
            ...
            if (res.data.modifiedCount > 0) {
                const message = status === 'In Progress' ? 'Bid Request Accepted' : 'Bid Request Rejected'
    ...
    const handleStatus = async (id, prevStatus, status) => {
        if (prevStatus === status) return toast.error("Already Updated");
    ...
    return (
        <section className='container px-4 mx-auto pt-12 min-h-[calc(100vh-418px)]'>
                                    ...
                                    {BidsRequest.map(bid => <tr key={bid._id}>
                                        ...
                                        <td className='px-4 py-4 text-sm whitespace-nowrap'>
                                            <div className='flex items-center gap-x-2'>
                                                ...
                                                ${bid.category === 'Digital Marketing' && 'text-pink-500 bg-pink-100/60'} text-xs  rounded-full`}
                                                >{bid.category}
                                                </p>
                                            </div>
                                        </td>
